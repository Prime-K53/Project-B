...
export const getPaymentNarrative = (calc: PaymentCalculation, customerName: string, date: string, invoiceNumber: string, currency: string = 'K') => {
    ...
    if (calc.paymentStatus === 'PARTIALLY PAID') {
        return `This is to acknowledge that Prime ERP has received a payment of ${currency} ${calc.amountPaid.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} from ${customerName} on ${formattedDate}. This payment has been applied toward Invoice ${invoiceNumber}.

Invoice Amount: ${currency} ${calc.invoiceTotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
Amount Paid: ${currency} ${calc.amountPaid.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
Outstanding Balance: ${currency} ${calc.outstandingBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}

This receipt confirms a partial settlement of the invoice.`;
    }

    return `Receipt acknowledgment for payment of ${currency} ${calc.amountPaid.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} received from ${customerName} on ${formattedDate} for Invoice ${invoiceNumber}.`;
};
