    ...
    async _processMarketAdjustments(
        ...
        for (const item of items) {
            ...
            if (hasTransactionSnapshots) {
                ...
                // âœ… Convert basic adjustmentSnapshots to TransactionAdjustmentSnapshot format
    ...
    async convertQuotationToInvoice(quotationId: string, invoiceData: Invoice) {
        return dbService.executeAtomicOperation(
            ...
            async (tx) => {
                ...
                const timestamp = new Date().toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
