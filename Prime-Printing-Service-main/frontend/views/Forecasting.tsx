          ...
          <div className="animate-fadeIn space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                ...
                <h3 className="text-2xl font-black text-slate-900 mt-1">{currency}{cashFlowForecast.currentCash.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h3>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <h3 className="text-2xl font-black text-emerald-600 mt-1">+{currency}{cashFlowForecast.totalInflow.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h3>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <h3 className="text-2xl font-black text-rose-600 mt-1">-{currency}{cashFlowForecast.totalOutflow.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h3>
              ...
              <div className={`p-6 rounded-2xl shadow-sm border border-transparent ${cashFlowForecast.minBalance < 0 ? 'bg-red-50 border-red-200' : 'bg-emerald-50 border-emerald-200'}`}>
                ...
                <h3 className={`text-2xl font-black mt-1 ${cashFlowForecast.minBalance < 0 ? 'text-red-700' : 'text-emerald-700'}`}>
                    {currency}{cashFlowForecast.minBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </h3>
                    ...
                    <ResponsiveContainer width="100%" height="100%" minHeight={150} minWidth={0}>
                        <AreaChart data={cashFlowForecast.timeline}>
                            ...
                            <Tooltip 
                                ...
                                formatter={(val: number) => [currency + val.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }), '']}
                            />
                            <Area 
                                type="monotone" 
                                dataKey="balance" 
                    ...
                    <div className="space-y-4">
                        {(invoices || []).filter(i => i.status !== 'Paid').slice(0, 5).map(inv => (
                            <div key={inv.id} className="flex justify-between items-center text-sm">
                                ...
                                <span className="font-mono font-bold text-emerald-600">+{currency}{(inv.totalAmount - (inv.paidAmount || 0)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="space-y-4">
                        {(purchases || []).filter(p => p.status !== 'Paid').slice(0, 5).map(p => (
                            <div key={p.id} className="flex justify-between items-center text-sm">
                                ...
                                <span className="font-mono font-bold text-rose-600">-{currency}{(p.total - (p.paidAmount || 0)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                            </div>
                        ))}
                    </div>
                </div>
