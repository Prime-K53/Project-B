        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 border border-white/60">
          ...
          <div className="p-6 space-y-6 text-[12px]">
            <div className="flex justify-between items-center p-4 bg-red-50 rounded-xl border border-red-100">
               <div className="flex items-center gap-3"><div className="p-2 bg-red-100 text-red-600 rounded-lg"><DollarSign size={24} /></div><div><p className="text-[10px] font-bold text-red-800 uppercase">Total Amount</p><p className="text-2xl font-bold text-slate-900">{currency}{selectedExpense.amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p></div></div>
    ...
    <div className="flex flex-col h-full max-w-[1600px] mx-auto font-normal">
      ...
      <div className="px-6 mb-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 shrink-0">
            {[
                { label: `Total Spend (${dateFilter})`, value: `${currency}${stats.total.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, icon: TrendingUp, color: 'blue' },
      ...
      <div className="flex-1 min-h-0 overflow-hidden flex flex-col lg:flex-row px-6 pb-6 gap-6 text-[12px]">
                  ...
                  <tbody className="divide-y divide-slate-100/50">
                     ...
                     {filteredExpenses.map(exp => (
                        <tr key={exp.id} className="hover:bg-blue-50/30 cursor-pointer group transition-colors">
                           ...
                           <td className="px-4 py-2.5 font-medium text-slate-800" onClick={() => setSelectedExpense(exp)}>{exp.description}<div className="text-[10px] text-slate-400 font-normal flex gap-1 items-center">{exp.id} â€¢ By {exp.recordedBy}{exp.paymentProofUrl && <Paperclip size={10} className="text-blue-500"/>}</div></td>
         ...
         <div className="w-full lg:w-80 bg-slate-50/50 rounded-2xl border border-slate-200/60 flex flex-col overflow-y-auto shadow-inner custom-scrollbar p-4 gap-4">
            <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm relative shrink-0"><h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2 text-xs uppercase tracking-wider"><PieChart size={14}/> Category Breakdown</h3><div style={{ width: '100%', height: 192, minHeight: 150 }} className="relative"><ResponsiveContainer width="100%" height="100%" minHeight={150} minWidth={0}><RePieChart><Pie data={stats.chartData} cx="50%" cy="50%" innerRadius={40} outerRadius={60} paddingAngle={5} dataKey="value">{stats.chartData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}</Pie><ReTooltip formatter={(val: number) => `${currency}${val.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} /><Legend verticalAlign="bottom" height={36} iconSize={8} wrapperStyle={{fontSize:'10px'}}/></RePieChart></ResponsiveContainer><div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none pb-8"><div className="text-[9px] text-slate-400 font-bold uppercase">Total</div><div className="text-xs font-bold text-slate-800">{currency}{stats.total.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div></div></div></div>
            <div className="space-y-3 shrink-0"><h3 className="font-bold text-slate-800 text-xs uppercase tracking-wider flex items-center gap-2"><AlertTriangle size={14} className="text-amber-500"/> Smart Insights</h3>{stats.highestExpense && <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm"><div className="text-[10px] font-bold text-red-600 uppercase mb-1 flex items-center gap-1"><ArrowUpRight size={10}/> High Value</div><p className="text-[12px] text-slate-600 mb-1 leading-tight">Largest single expense: <b>{stats.highestExpense.description}</b>.</p><div className="text-sm font-bold text-slate-900">{currency}{stats.highestExpense.amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div></div>}<div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm"><div className="text-[10px] font-bold text-blue-600 uppercase mb-1 flex items-center gap-1"><FileText size={10}/> Top Category</div><p className="text-[12px] text-slate-600 leading-tight"><b>{stats.chartData.length > 0 ? stats.chartData[stats.chartData.length-1].name : 'None'}</b> accounts for the majority of costs.</p></div></div>
         </div>
      </div>
    </div>
  );
