                ...
                {showCompare && !isTotal && (
                    <div className="flex flex-col items-end min-w-[100px]">
                        <span className="text-xs text-slate-400">
                            {prevAmount < 0 ? `(${currency}${Math.abs(prevAmount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })})` : `${currency}${prevAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
                        </span>
    ...
    const handlePreviewReport = () => {
        ...
        if (reportType === 'IncomeStatement') {
            ...
            reportData.sections = budgetData.reduce((acc: any[], item) => {
                ...
                section.rows.push({
                    label: item.name,
                    subText: `${item.code} â€¢ Budget: ${currency}${item.budget.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
                    amount: item.actual,
                ...
                <table className="w-full text-left text-xs border-collapse">
                        ...
                        {budgetData.map(item => {
                            ...
                            return (
                                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="py-4 px-2 text-right font-mono font-bold">{currency}{item.actual.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                    <td className="py-4 px-2 text-right font-mono text-slate-500">{currency}{item.budget.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                    <td className={`py-4 px-2 text-right font-mono font-bold ${isFavorable ? 'text-emerald-600' : 'text-rose-600'}`}>
                                        {item.variance > 0 ? '+' : ''}{currency}{item.variance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                    </td>
                    ...
                    <tfoot className="border-t-4 border-double border-slate-900 bg-slate-50">
                        <tr className="font-bold">
                            ...
                            <td className="py-4 px-2 text-right font-mono">{currency}{budgetData.reduce((s, i) => s + i.actual, 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                            <td className="py-4 px-2 text-right font-mono text-slate-500">{currency}{budgetData.reduce((s, i) => s + i.budget, 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                        ...
                        <tfoot className="border-t-4 border-double border-slate-900 bg-slate-50">
                            <tr className="font-bold">
                                ...
                                <td className="py-4 px-2 text-right text-lg">{currency}{Object.values(data.buckets).reduce((s, a) => s + (a as number), 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
                ...
                <div className="w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
                    <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                        <div>
                            ...
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">GL Source â€¢ {drilldownAccount.code}</p>
                        </div>
                                ...
                                {accountEntries.map(entry => {
                                    ...
                                    return (
                                        <tr key={entry.id} className="hover:bg-slate-50">
                                            ...
                                            <td className="py-3 text-right font-mono font-bold text-slate-600">{isDebit ? entry.amount.toFixed(2) : 'â€”'}</td>
                                            <td className="py-3 text-right font-mono font-bold text-slate-600">{!isDebit ? entry.amount.toFixed(2) : 'â€”'}</td>
                                        </tr>
                                    );
                                })}
                                                        ...
                                                        {a.isInventory && Math.abs(drift) > 1 && (
                                                            <div className="mx-2 mb-4 p-4 bg-amber-50 rounded-xl border border-amber-100">
                                                                <div className="flex items-center justify-between mb-2">
                                                                    ...
                                                                    <div className="flex items-center gap-3">
                                                                        <span className={`text-[10px] font-black ${drift > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                                                                            {drift > 0 ? '+' : ''}{currency}{drift.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                                        </span>
                                                                        <button
                                                                            onClick={(e) => {
                                                                                e.stopPropagation();
                                                                ...
                                                                <div className="flex justify-between text-[9px] font-bold text-amber-600/70">
                                                                    <span>Physical: {currency}{physicalValuation.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                                                    <span>Ledger: {currency}{a.balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                                                </div>
                                                            </div>
                                                        )}
                                                    </div>
                                        ...
                                        <tbody className="divide-y divide-slate-100">
                                            {accounts.map(a => {
                                                ...
                                                return (
                                                    <tr key={a.id} className="hover:bg-slate-50 transition-colors">
                                                        ...
                                                        <td className="py-3 text-right tabular-nums font-mono">{dr > 0 ? dr.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : 'â€”'}</td>
                                                        <td className="py-3 text-right tabular-nums font-mono">{cr > 0 ? cr.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : 'â€”'}</td>
                                                    </tr>
                                                );
                                            })}
                                            <tr className="border-t-4 border-double border-[#393A3D] font-bold bg-slate-50">
                                                ...
                                                <td className="py-4 text-right tabular-nums font-mono">
                                                    {currency}{accounts.reduce((sum, a) => sum + (accountBalances.current[a.id] > 0 ? accountBalances.current[a.id] : 0), 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                </td>
                                                <td className="py-4 text-right tabular-nums font-mono">
                                                    {currency}{accounts.reduce((sum, a) => sum + (accountBalances.current[a.id] < 0 ? Math.abs(accountBalances.current[a.id]) : 0), 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
