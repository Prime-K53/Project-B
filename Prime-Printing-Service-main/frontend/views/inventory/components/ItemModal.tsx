    ...
    const calculateItemFinancials = (pPages: number, pConfig: PricingConfig | undefined, pItemType?: string, pManualCost?: number) => {
        ...
        if (pItemType === 'Stationery') {
            ...
            finishingCost = pConfig.finishingOptions.reduce((acc, option) => {
                ...
                if (mat && option.coversPerCopy && option.coversPerCopy > 0) {
                    const capacity = mat.rollLength || mat.conversionRate || 1;
                    const unitCost = mat.cost / capacity;
                    // Calculate cost per copy: (coversPerCopy / capacity) * materialCost
                    const costPerCopy = (option.coversPerCopy / capacity) * mat.cost;
                    return acc + costPerCopy;
                }
                return acc;
            }, 0);
                                                ...
                                                <div className="space-y-3">
                                                    {formData.pricingConfig?.finishingOptions.map((option, idx) => (
                                                        <div key={option.id || idx} className="flex gap-3 items-end bg-white p-3 rounded border border-slate-200">
                                                            ...
                                                            <div className="w-28">
                                                                <label className="block text-[10px] text-slate-500 mb-1">Units/Copy</label>
                                                                <input
                                                                    type="number"
                                                                    step="0.01"
                                                                    min="0"
                                                                    className="w-full px-2 py-1.5 border border-slate-200 rounded text-sm"
                                                                    value={option.coversPerCopy || 0}
                                                                    onChange={(e) => updateFinishingOption(option.id, 'coversPerCopy', Number(e.target.value))}
                                                                />
                                                            </div>
                                                        </div>
                                                    ))}
