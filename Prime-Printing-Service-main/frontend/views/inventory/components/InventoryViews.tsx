                                ...
                                <React.Fragment key={`${item.id}-${item.sku}`}>
                                    ...
                                    >
                                        ...
                                        <td className="table-body-cell text-center finance-nums font-bold text-slate-600">
                                            {item.type === 'Service' ? <span className="text-slate-300">-</span> : (
                                                <>
                                                    {item.stock.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">{item.unit}</span>
                                    ...
                                    {isExpanded && hasVariants && item.variants?.map((variant: any) => (
                                        <tr key={variant.id} id={`variant-${variant.id}`} className="bg-slate-50/50 hover:bg-blue-50/30 transition-colors border-l-4 border-blue-400 group/variant">
                                            ...
                                            <td className="table-body-cell text-center finance-nums font-bold text-slate-600">
                                                {variant.stock.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">{item.unit}</span>
                                            </td>
                                            <td className={`table-body-cell text-right finance-nums font-bold ${item.type === 'Material' ? 'text-red-600' : 'text-blue-600'}`}>
                    ...
                    <div key={wh.id} className="glass-card p-6 rounded-2xl hover:shadow-float transition-all duration-300 group border border-white/60">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6">
                            <div className="p-3 bg-slate-50/50 rounded-xl text-center border border-slate-100">
                                <div className="text-[13px] font-bold text-slate-800 finance-nums">{stockCount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                                <div className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Units</div>
                            </div>
                            <div className="p-3 bg-slate-50/50 rounded-xl text-center border border-slate-100">
                                <div className="text-[13px] font-bold text-slate-800 finance-nums">{distinctItems.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
