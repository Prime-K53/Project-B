          ...
          {preview && (
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-2 mt-4 text-[11px] font-bold text-indigo-900 shadow-sm animate-in fade-in slide-in-from-top-2 duration-300 flex flex-col">
              <div className="flex flex-wrap items-center justify-center gap-2 text-center text-sm">
                ...
                <span className="text-slate-900 inline-flex items-center">{preview.totalSheets.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Sheets <span className="text-indigo-400 ml-1 font-normal">({preview.totalPages.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} pgs)</span></span>
                        ...
                        {paperMaterials.map((m) => {
                          ...
                          return (
                            <option key={m.id} value={String(m.id)}>
                              {m.name} ({companyConfig?.currencySymbol || 'MWK'}{cost > 0 ? cost.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0'}/ream, {sheetsPerReam} sheets)
                            </option>
                          );
                        })}
                      </select>
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        ...
                        {tonerMaterials.map((m) => {
                          ...
                          return (
                            <option key={m.id} value={String(m.id)}>
                              {m.name} ({companyConfig?.currencySymbol || 'MWK'}{cost > 0 ? cost.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0'}/kg, {pagesPerKg.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} pages)
                            </option>
                          );
                        })}
                      </select>
