...
export const BOMDialog: React.FC<BOMDialogProps> = ({ isOpen, onClose, batch }) => {
  ...
  const exportCsv = useCallback(() => {
    ...
    lines.push([csv('Last Synced'), csv(lastSyncedAt ? new Date(lastSyncedAt).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '')].join(','));
    lines.push('');
  ...
  const buildJobCardHtml = useCallback((generatedAt?: string) => {
    ...
    const generated = generatedAt ? new Date(generatedAt).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : new Date().toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-2 text-[13px] leading-[1.45] text-slate-500">
                  ...
                  <span>Last synchronized: {lastSyncedAt ? new Date(lastSyncedAt).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : 'Not synced yet'}</span>
                </div>
              </div>
                  ...
                  <div className="rounded-xl border border-amber-200/60 bg-gradient-to-r from-amber-50/80 to-amber-100/40 p-4 shadow-lg shadow-amber-500/5">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-amber-700">{mismatchCount} row(s) had a variance between quantity Ã— unit cost and stored BOM totals.</div>
                      </div>
                    </div>
                  </div>
                )}
                                    ...
                                    {isProvenanceMaterial(g.materialType) && g.costSource && (
                                      <div className="text-xs text-slate-400">
                                        ...
                                        <span className="text-xs text-slate-400">
                                          ...
                                          {g.sourceTimestamp && ` â€¢ ${new Date(g.sourceTimestamp).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
                                        </span>
                                      </div>
                                    )}
                                  </div>
          ...
          <DialogHeader>
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-0.5 text-xs text-slate-500">Generated: {previewAt ? new Date(previewAt).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : new Date().toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              </div>
