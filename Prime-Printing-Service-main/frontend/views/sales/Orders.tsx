                ...
                if (window.confirm(`Convert Order #${item.orderNumber} to an Invoice?`)) {
                    try {
                        ...
                        const newInvoice: Invoice = {
                            ...
                            notes: `Converted from [Order] #[${item.orderNumber}] on [${new Date().toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}] as accepted by [${user?.name || 'System'}]`,
                    ...
                    try {
                        for (const id of selectedInvoiceIds) {
                            ...
                            const invoiceData = {
                                ...
                                status: 'Unpaid',
                                notes: `Converted from [Order] #[${order.orderNumber}] on [${new Date().toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}] as accepted by [${user?.name || 'System'}]`,
        ...
        <div className="p-4 md:p-6 max-w-[1600px] mx-auto h-[calc(100vh-4rem)] flex flex-col relative w-full text-sm font-normal">
                    ...
                    <div className="bg-white rounded-3xl shadow-premium w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-white/40 flex flex-col h-[80vh]">
                        <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center shrink-0">
                            <div>
                                ...
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mt-1">Status Verified â€¢ No Spoilers</p>
                            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 shrink-0">
                {activeView === 'Invoices' ? (
                    <>
                        {[
                            { label: 'Total Invoiced', value: `${currency}${formatNumber(invoiceStats.total)} `, icon: TrendingUp, color: 'blue' },
                            { label: 'Annual Profit', value: `${currency}${formatNumber(invoiceStats.annualProfit)} `, icon: TrendingUp, color: 'emerald' },
                            { label: 'Outstanding', value: `${currency}${formatNumber(invoiceStats.outstanding)} `, icon: Wallet, color: 'indigo' },
                            { label: 'Overdue Amount', value: `${currency}${formatNumber(invoiceStats.overdue)} `, icon: AlertCircle, color: 'rose' }
                        ].map((item, idx) => (
                        ...
                        <ResponsiveContainer width="100%" height="100%" minHeight={150} minWidth={0}>
                            <BarChart data={dashboardData.monthly}>
                                    ...
                                    <Tooltip
                                        ...
                                        formatter={(val) => [`${currency}${formatNumber(Number(val))}`]}
                                    />
