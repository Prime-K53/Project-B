...
import { generateNextId, getDefaultPaymentTermsForSegment, resolveCustomerPaymentPolicy, roundToCurrency, formatNumber } from '../../../utils/helpers';
import { pricingService, DynamicServicePricingResult } from '../../../services/pricingService';
import { dbService } from '../../../services/db';

import { useNavigate } from 'react-router-dom';
import { VariantSelectorModal, ServiceCalculatorModal, QuickPrintModal } from '../../pos/components/PosModals';
...
export const OrderForm: React.FC<OrderFormProps> = ({ type, initialData, onSave, onCancel, onPreview }) => {
    ...
    const [selectedQuickPrintService, setSelectedQuickPrintService] = useState<Item | null>(null);
    ...
    // Use inventory cost price and selling price as final â€” no recalculation
    ...
    const handleQuickPhotocopy = () => {
        ...
        setSelectedQuickPrintService(photocopyItem);
    };
    ...
    const handleQuickTypePrinting = () => {
        ...
        setSelectedQuickPrintService(printItem);
    };
    ...
    const handleAddItem = async (item: Item, bypassCalculator = false) => {
        if (item.isVariantParent) {
            setSelectedProductForVariants(item);
            return;
        }

        if (item.type === 'Service' && !bypassCalculator) {
    ...
    const handleVariantSelect = async (variant: ProductVariant) => {
        ...
        if (existingItemIdx > -1) {
            ...
            const variantItem: any = {
                ...
                // âœ… Variant-specific adjustment data for margin tracking
            ...
            // Use variant's inventory price directly â€” no recalculation
            ...
            // âœ… Use calculated adjustmentSnapshots from getInventoryPrices
    ...
    const handlePagesChange = (idx: number, newPages: number) => {
        ...
        // Price stays as inventory price â€” no recalculation
        setFormData({ ...formData, items: newItems });
    };
    ...
    const selectCustomer = (name: string) => {
        ...
        if (customer && customer.creditLimit) {
            ...
            if (outstanding > customer.creditLimit) {
                notify(`Warning: ${selectedName} has exceeded their credit limit. Outstanding: ${currency}${formatNumber(outstanding)}`, "warning");
            }
        }
    };
    ...
    return (
        <>
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="text-right">
                                                                        <div className={`text-[11px] font-semibold ${outstanding > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                                                                            {currency}{formatNumber(outstanding)}
                                                                        </div>
                                                                    </div>
                                                                </button>
                                                            );
                                    ...
                                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-right">
                                                ...
                                                <p className="mt-1 text-lg font-semibold text-slate-900">{formatNumber(examinationLearnerCount)}</p>
                                            </div>
                                            <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-right">
                                                ...
                                                <p className="mt-1 text-lg font-semibold text-blue-700">
                                                    {currency}{formatNumber(analysis.totalAmount)}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={e => {
                                                            if (e.target.checked && selectedCustomerObj) {
                                                                ...
                                                                if (balance < 1000) {
                                                                    notify(`Auto-Deduct requires a wallet balance of at least ${currency}1,000. ${accountLabel} current: ${currency}${formatNumber(balance)}`, "error");
                                                                    return;
                                                                }
                                                            }
                            ...
                            {isExaminationQuotation ? (
                                ...
                                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-5">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <button onClick={handleQuickPhotocopy} type="button" className="text-blue-600 hover:bg-blue-50 px-2 py-1.5 rounded-lg flex items-center gap-1.5 text-[11px] font-semibold transition-colors">
                                                <Copy size={14} /> Photocopy
                                            </button>
                                            <button onClick={handleQuickTypePrinting} type="button" className="text-blue-600 hover:bg-blue-50 px-2 py-1.5 rounded-lg flex items-center gap-1.5 text-[11px] font-semibold transition-colors">
                                                ...
                                                return (
                                                    <tr key={idx} className="hover:bg-blue-50/30 transition-colors group">
                                                        ...
                                                        <td className="py-2 px-5 text-right font-normal text-slate-900 font-mono text-[13px]">
                                                            {currency}{formatNumber((Number(item.price) || 0) * (Number(item.quantity) || 0))}
                                                        </td>
                            ...
                            <div className="mb-auto space-y-6 pb-6">
                                ...
                                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
                                    {selectedCustomerObj && (
                                        <div className={`p-3 rounded-xl border mb-4 ${getCustomerOutstanding(selectedCustomerObj.name) + finalDisplayTotal > (selectedCustomerObj.creditLimit || 0) && selectedCustomerObj.creditLimit ? 'bg-rose-500/10 border-rose-500/50' : 'bg-slate-50 border-slate-100'}`}>
                                            ...
                                            <div className="flex justify-between items-end">
                                                <div className="text-[9px] text-slate-500 font-normal uppercase tracking-tighter">Limit: {currency}{formatNumber(selectedCustomerObj.creditLimit || 0)}</div>
                                                <div className="text-xs font-normal text-slate-700">Exposure: {currency}{formatNumber(getCustomerOutstanding(selectedCustomerObj.name) + finalDisplayTotal)}</div>
                                            </div>
                                        </div>
                                    )}
                                    <div className="flex justify-between items-center text-[15px] font-normal">
                                        <span className="text-slate-400">Net Valuation</span>
                                        <span className="text-slate-700 font-mono">{currency}{formatNumber(analysis.totalCostPrice)}</span>
                                    </div>
                                            ...
                                            return (
                                                <div key={name} className="flex justify-between items-center">
                                                    <span className="text-slate-400 text-[11px] font-normal tracking-tight flex items-center gap-1.5">
                                                        <Icon size={10} className={colorClass} /> â€¢ {name}
                                                    </span>
                                                    <span className={`${textClass} font-mono text-[11px] font-medium`}>+{currency}{formatNumber(amount as number)}</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                                    ...
                                    <div className="pt-6 mt-2 border-t-2 border-slate-900 flex justify-between items-center">
                                        ...
                                        <span className="text-2xl font-semibold text-blue-600 tracking-tighter leading-none">{currency}{formatNumber(finalDisplayTotal)}</span>
                                    </div>
                                </div>
            ...
            {selectedQuickPrintService && (
                <QuickPrintModal
                    service={selectedQuickPrintService}
                    onClose={() => setSelectedQuickPrintService(null)}
                    onSelect={(item) => {
                        handleAddItem(item, true);
                        setSelectedQuickPrintService(null);
                        notify(`${item.name} added`, 'success');
                    }}
                />
            )}
