                                            ...
                                            <tbody className="divide-y divide-slate-200/50">
                                                {order.items.map((item, idx) => (
                                                    <tr key={idx} className="hover:bg-white transition-colors">
                                                        ...
                                                        <td className="px-6 py-4 text-right font-medium text-slate-600">{currency}{item.unitPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                                        <td className="px-6 py-4 text-right font-black text-slate-900">{currency}{item.subtotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                                    ...
                                    <div className="grid grid-cols-2 gap-8">
                                        ...
                                        <div className="bg-slate-900 text-white p-8 rounded-[2rem] shadow-xl space-y-4">
                                            <div className="flex justify-between items-center text-slate-400 text-sm font-medium">
                                                <span>Subtotal</span>
                                                <span>{currency}{((order.totalAmount || 0) - (order.tax || 0)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                            </div>
                                            {order.tax && order.tax > 0 && (
                                                <div className="flex justify-between items-center text-slate-400 text-sm font-medium">
                                                    <span>Tax ({order.taxRate}%)</span>
                                                    <span>{currency}{order.tax.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                                </div>
                                            )}
                                            <div className="h-px bg-white/10 my-2" />
                                            <div className="flex justify-between items-center">
                                                <span className="text-sm font-bold uppercase tracking-widest text-indigo-400">Total Amount</span>
                                                <span className="text-3xl font-black">{currency}{order.totalAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                            </div>
                                            <div className="flex justify-between items-center pt-4">
                                                <span className="text-xs font-medium text-emerald-400">Paid Amount</span>
                                                <span className="text-xl font-bold text-emerald-400">{currency}{order.paidAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                            </div>
                                            <div className="flex justify-between items-center">
                                                <span className="text-xs font-medium text-rose-400">Balance Due</span>
                                                <span className="text-xl font-bold text-rose-400">{currency}{order.remainingBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            {order.payments.map((payment, idx) => (
                                                <div key={idx} className="bg-white border border-slate-200 p-6 rounded-3xl flex justify-between items-center hover:shadow-md transition-shadow">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <div className="font-black text-slate-900">{currency}{payment.amountPaid.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                                                            <div className="text-xs text-slate-500 font-medium">
                                                                via {payment.paymentMethod} â€¢ {new Date(payment.paymentDate).toLocaleDateString()}
                                                            </div>
                                                        </div>
                                                    </div>
