                  ...
                  <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col justify-center">
                    <div className="space-y-1">
                      <div className="flex justify-between text-sm text-slate-500">
                        <span>Subtotal</span>
                        <span className="font-bold">{currency}{((quotation.total || 0) - (quotation.tax || 0)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                      </div>
                      {quotation.tax && quotation.tax > 0 && (
                        <div className="flex justify-between text-sm text-slate-500">
                          <span>Tax ({quotation.taxRate}%)</span>
                          <span className="font-bold">{currency}{quotation.tax.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                      )}
                      ...
                      <div className="flex justify-between items-end">
                        ...
                        <p className="text-3xl font-black text-slate-900 tabular-nums leading-none">
                          {currency}{quotation.total.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </p>
                      </div>
                    </div>
                  </div>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {quotation.items?.map((item, idx) => (
                          <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="px-6 py-4 text-right font-bold text-slate-700">{currency}{item.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                            <td className="px-6 py-4 text-right font-bold text-slate-900">{currency}{(item.quantity * item.price).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                {((quotation as any).adjustmentSnapshots && (quotation as any).adjustmentSnapshots.length > 0) || ((quotation as any).adjustmentBreakdown && (quotation as any).adjustmentBreakdown.length > 0) ? (
                  <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
                    <div className="px-6 py-4 border-b border-slate-100 bg-indigo-50/30 flex justify-between items-center">
                      ...
                      <span className="text-xs font-black text-indigo-600">Total: {currency}{((quotation as any).adjustmentTotal || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                    <div className="p-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {((quotation as any).adjustmentSnapshots || []).map((snap: any, idx: number) => (
                          <div key={idx} className="flex justify-between items-center p-4 bg-indigo-50/30 border border-indigo-100/50 rounded-2xl">
                            ...
                            <div className="text-right">
                              <p className="text-sm font-black text-slate-900">+{currency}{snap.calculatedAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                            </div>
                          </div>
                        ))}
                        {(!((quotation as any).adjustmentSnapshots) || (quotation as any).adjustmentSnapshots.length === 0) && ((quotation as any).adjustmentBreakdown || []).map((adj: any, idx: number) => (
                          <div key={idx} className="flex justify-between items-center p-4 bg-indigo-50/30 border border-indigo-100/50 rounded-2xl">
                            ...
                            <div className="text-right">
                              <p className="text-sm font-black text-slate-900">+{currency}{adj.amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                            </div>
                          </div>
                        ))}
                      </div>
