            ...
            <div className="bg-white rounded-[1.5rem] shadow-2xl w-full max-w-4xl h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200/60 font-sans text-[13px] leading-relaxed text-slate-800">
                ...
                <div className="grid grid-cols-3 bg-white border-b border-slate-100 shrink-0">
                    <div className="p-4 text-center border-r border-slate-100">
                        ...
                        <p className="text-[20px] font-semibold text-slate-800 tabular-nums">{currency}{totalAmountDisplay.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                    </div>
                    <div className="p-4 text-center border-r border-slate-100">
                        <p className="text-[12.5px] font-medium text-slate-500 tracking-wide mb-0.5">Payments</p>
                        <p className="text-[20px] font-semibold text-emerald-600 tabular-nums">{currency}{paidAmountDisplay.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '0.00'}</p>
                    </div>
                    <div className="p-4 text-center">
                        <p className="text-[12.5px] font-medium text-slate-500 tracking-wide mb-0.5">Net balance</p>
                        <p className={`text-[20px] font-semibold tabular-nums ${(balanceDue || 0) > 0.001 ? 'text-rose-600' : 'text-slate-300'}`}>{currency}{(balanceDue || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                    </div>
                </div>
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="mt-1 flex items-center gap-3 text-[12.5px] text-slate-500">
                                                            <span className="flex items-center gap-1.5">
                                                                <Clock size={12} />
                                                                {new Date((invoice as any).conversionDetails.date).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                            </span>
                                    ...
                                    <div className="bg-white p-5 rounded-[1.25rem] border border-slate-200 shadow-sm">
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
                                                ...
                                                <span className="text-[13px] font-semibold text-slate-700 tabular-nums">{new Date(invoice.date).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                            </div>
                                            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
                                                <span className="text-[12.5px] font-medium text-slate-500">Last modified</span>
                                                <span className="text-[13px] font-semibold text-slate-700 tabular-nums">{new Date((invoice as any).updatedAt || invoice.date).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            {(invoice.adjustmentSnapshots && invoice.adjustmentSnapshots.length > 0) || (invoice.adjustmentBreakdown && invoice.adjustmentBreakdown.length > 0) ? (
                                <div className="bg-white rounded-[1.25rem] border border-slate-200 overflow-hidden shadow-sm">
                                    <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                                        ...
                                        <span className="text-[12px] font-bold text-indigo-600">Total: {currency}{(invoice.adjustmentTotal || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                    </div>
                                    <div className="p-6">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {(invoice.adjustmentSnapshots || []).map((snap: any, idx: number) => (
                                                <div key={idx} className="flex justify-between items-center p-3 bg-indigo-50/30 border border-indigo-100/50 rounded-xl">
                                                    ...
                                                    <div className="text-right">
                                                        <p className="text-[13px] font-black text-slate-900">+{currency}{snap.calculatedAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                                                    </div>
                                                </div>
                                            ))}
                                            {(!invoice.adjustmentSnapshots || invoice.adjustmentSnapshots.length === 0) && (invoice.adjustmentBreakdown || []).map((adj: any, idx: number) => (
                                                <div key={idx} className="flex justify-between items-center p-3 bg-indigo-50/30 border border-indigo-100/50 rounded-xl">
                                                    ...
                                                    <div className="text-right">
                                                        <p className="text-[13px] font-black text-slate-900">+{currency}{adj.amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                                ...
                                                ledger.filter(entry => entry.reference === invoice.id).map((entry, idx) => (
                                                    <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                                        ...
                                                        <td className="px-6 py-4 text-right font-semibold text-slate-700 tabular-nums">
                                                            {entry.type === 'Debit' ? `${currency}${entry.amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '-'}
                                                        </td>
                                                        <td className="px-6 py-4 text-right font-semibold text-slate-700 tabular-nums">
                                                            {entry.type === 'Credit' ? `${currency}${entry.amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '-'}
                                                        </td>
                                                    </tr>
                                                ))
                                            ) : (
                                            ...
                                            {paymentHistory.map(payment => (
                                                <tr key={payment.id} className="hover:bg-slate-50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4 text-right font-semibold text-emerald-600 tabular-nums">
                                                        {currency}{(payment.allocations?.find(a => a.invoiceId === invoice.id)?.amount || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                    </td>
                                                ...
                                                <div key={log.id} className="flex gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="flex justify-between items-start mb-1">
                                                            ...
                                                            <span className="text-[12px] font-medium text-slate-400 tabular-nums">{new Date(log.date).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                                        </div>
                            ...
                            <div className="p-5 max-h-[50vh] overflow-y-auto space-y-2 custom-scrollbar">
                                {availableCredits.map(payment => (
                                    <div key={payment.id} onClick={() => handleAllocateCredit(payment)} className="flex justify-between items-center p-4 border border-slate-100 rounded-xl hover:border-emerald-400 hover:bg-emerald-50 transition-all cursor-pointer group bg-white shadow-sm">
                                        ...
                                        <div className="text-right">
                                            <div className="text-[16px] font-semibold text-emerald-600 group-hover:scale-105 transition-transform tabular-nums">{currency}{(payment.creditApplied || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
