        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div className="p-6 bg-blue-50 rounded-3xl border border-blue-100 shadow-sm">
                    ...
                    <div className="text-2xl font-black text-blue-900">{currency}{(analysis.totalRevenue || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                </div>
                <div className="p-6 bg-slate-900 rounded-3xl shadow-xl text-white">
                    ...
                    <div className="text-2xl font-black">{currency}{(analysis.totalCost || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                ...
                <div className="p-6 bg-emerald-50 rounded-3xl border border-emerald-100 shadow-sm">
                    ...
                    <div className={`text-2xl font-black ${analysis.totalProfit >= 0 ? 'text-emerald-900' : 'text-rose-600'}`}>{currency}{(analysis.totalProfit || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                </div>
                        ...
                        {analysis.snapshots.map((adj, idx) => (
                            <div key={idx} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                ...
                                <div className="text-lg font-black text-slate-900">
                                    {currency}{adj.calculatedAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </div>
                            </div>
                        ))}
                    </div>
                        ...
                        {analysis.items.map((item, idx) => (
                            <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className={`p-5 text-right font-black ${item.profit >= 0 ? 'text-slate-900' : 'text-rose-600'} font-mono`}>
                                    {currency}{(item.profit || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </td>
                            </tr>
                        ))}
                    </tbody>
