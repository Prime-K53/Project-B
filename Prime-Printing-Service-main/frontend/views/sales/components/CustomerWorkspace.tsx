        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-500 font-medium">Customer ID: {customer.id} â€¢ {customer.category || 'Standard Client'}</p>
            </div>
          </div>
        </div>
                    ...
                    <div key={item.id + idx} className="relative flex items-center justify-between md:justify-start md:odd:flex-row-reverse group">
                      ...
                      <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border border-slate-100 bg-white group-hover:border-blue-200 transition-all shadow-sm ml-6">
                        ...
                        <div className="text-slate-500 text-[12px] font-medium">
                          {item.totalAmount !== undefined ?
                            `Invoiced amount: ${currency}${item.totalAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` :
                            `Received amount: ${currency}${item.amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
                        </div>
                      </div>
                    </div>
                  ))}
                    ...
                    <tbody className="divide-y divide-slate-50">
                      ...
                      {ledgerEntries.map((row, idx) => (
                        <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-right text-rose-600 finance-nums">{row.debit > 0 ? `${currency}${row.debit.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '-'}</td>
                          <td className="px-6 py-4 text-right text-emerald-600 finance-nums">{row.credit > 0 ? `${currency}${row.credit.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '-'}</td>
                          <td className="px-6 py-4 text-right font-bold text-slate-900 finance-nums">{currency}{row.runningBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                  ...
                  <div className="text-2xl font-black text-slate-900 finance-nums">
                    {currency}{customerWalletTransactions.filter(t => t.type === 'Deposit').reduce((sum, t) => sum + t.amount, 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </div>
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                  ...
                  <div className="text-2xl font-black text-slate-900 finance-nums">
                    {currency}{customerWalletTransactions.filter(t => t.type === 'Deduction').reduce((sum, t) => sum + t.amount, 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </div>
