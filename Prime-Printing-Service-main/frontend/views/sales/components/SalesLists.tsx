            ...
            <div className="bg-slate-800/95 backdrop-blur-md border border-slate-700 rounded-2xl shadow-premium p-4 min-w-[280px] flex flex-col gap-3 text-white">
                            ...
                            <div key={idx} className="flex justify-between items-start gap-4 text-xs py-1 border-b border-white/5 last:border-0">
                                <span className="text-slate-200 font-medium line-clamp-1">
                                    {type === 'SalesExchange' && item.qty_replaced > 0 && item.replaced_product_name && item.replaced_product_name !== item.product_name
                                        ? `${item.product_name} â†’ ${item.replaced_product_name}`
                ...
                <div className="mt-2 pt-2 border-t border-slate-700 flex justify-between items-center">
                    ...
                    <span className="text-[13px] font-bold text-emerald-400 finance-nums">
                        {currency}{formatNumber(data.total || data.totalAmount || data.total_price_difference || 0)}
                    </span>
                </div>
                            ...
                            <div key={item.id} className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 hover:shadow-md transition-shadow">
                                ...
                                <div className="space-y-2 mb-4">
                                    ...
                                    <div className="flex justify-between text-xs">
                                        ...
                                        <span className="font-bold text-slate-900">{companyConfig.currencySymbol}{formatNumber(item.totalAmount)}</span>
                                    </div>
                                    <div className="flex justify-between text-xs">
                                        <span className="text-slate-500">Paid:</span>
                                        <span className="font-bold text-emerald-600">{companyConfig.currencySymbol}{formatNumber(item.paidAmount)}</span>
                                    </div>
                                </div>
                            ...
                            <tbody className="divide-y divide-slate-100/50">
                                {(currentItems || []).map((o: Order) => (
                                    ...
                                    >
                                        ...
                                        <td className="table-body-cell font-medium text-slate-900">{o.customerName}</td>
                                        <td className="table-body-cell text-right font-bold">{companyConfig.currencySymbol}{formatNumber(o.totalAmount)}</td>
                                        <td className="table-body-cell text-right font-bold text-emerald-600">{companyConfig.currencySymbol}{formatNumber(o.paidAmount)}</td>
                                        <td className="table-body-cell text-center">
                                            <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold border ${o.status === 'Completed' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' :
    ...
    if (type === 'JobOrder') {
        ...
        financials = (
            <div className="flex justify-between items-center text-[13px] mt-1">
                ...
                <span className="font-bold text-slate-800 finance-nums">{currencySymbol} {formatNumber(cost || 0)}</span>
            </div>
        );
    }
                                ...
                                {(currentItems || []).map((inv: any) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="table-body-cell text-right font-medium finance-nums">{companyConfig.currencySymbol} {formatNumber(totalAmount)}</td>
                                            <td className="table-body-cell text-right text-red-600 font-medium finance-nums">{companyConfig.currencySymbol} {formatNumber(balanceDue)}</td>
                                            <td className="table-body-cell text-center">
                            ...
                            <tbody className="divide-y divide-slate-100/50">
                                {(currentItems || []).map((q: any) => (
                                    ...
                                    >
                                        ...
                                        <td className="table-body-cell text-right font-medium finance-nums">{companyConfig.currencySymbol} {formatNumber(q.total || 0)}</td>
                                        <td className="table-body-cell text-center">
                        ...
                        <tbody className="divide-y divide-slate-100/50">
                            {(currentItems || []).map((sub: any) => (
                                ...
                                >
                                    ...
                                    <td className="table-body-cell text-right font-medium finance-nums">{companyConfig.currencySymbol} {formatNumber(sub.total || 0)}</td>
                                    <td className="table-body-cell text-center">
