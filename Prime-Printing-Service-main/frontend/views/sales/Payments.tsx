...
import { generateNextId, roundFinancial, formatNumber } from '../../utils/helpers';
            ...
            <div className="bg-[#000D1A]/95 backdrop-blur-md border border-white/20 rounded-2xl shadow-premium p-4 min-w-[200px] flex flex-col gap-3">
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex justify-between items-center text-[10px]">
                        ...
                        <span className="text-emerald-400 font-bold finance-nums">{currency}{formatNumber(payment.amount)}</span>
                    </div>
                    <div className="flex justify-between items-center text-[10px]">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                <div className="flex items-center justify-between p-6 bg-slate-50 rounded-2xl border border-slate-100 shadow-sm">
                    <div>
                        ...
                        <p className="text-3xl font-black text-slate-900 finance-nums">{currency}{formatNumber(payment.amount)}</p>
                    </div>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {(payment.allocations || []).map((a, i) => (
                                    <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td className="table-body-cell text-right font-bold finance-nums">{currency}{formatNumber(a.amount)}</td>
                                    </tr>
                                ))}
                                    ...
                                    <tfoot className="bg-slate-50/80 font-bold border-t border-slate-100">
                                        <tr>
                                            ...
                                            <td className="table-body-cell text-right text-slate-900 finance-nums">
                                                {currency}{formatNumber(allocated)}
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
                            ...
                            {ledger.filter(e => e.referenceId === payment.id).map(entry => (
                                <div key={entry.id} className="p-4 bg-white border border-slate-100 rounded-2xl shadow-sm hover:border-blue-200 transition-all group">
                                    <div className="flex justify-between items-start mb-3">
                                        ...
                                        <div className="text-[10px] font-black text-slate-900">{currency}{formatNumber(entry.amount)}</div>
                                    </div>
                                    ...
                                    {supplierPayments
                                        ...
                                        .map(payment => (
                                            ...
                                            >
                                                ...
                                                <td className="table-body-cell text-right font-bold text-slate-900 finance-nums">{currency}{formatNumber(payment.amount)}</td>
                                                <td className="table-body-cell text-right">
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                ...
                                                .map(bill => {
                                                    ...
                                                    return (
                                                        <tr key={bill.id}>
                                                            ...
                                                            <td className="px-4 py-2 text-slate-500">{currency}{formatNumber(due)}</td>
